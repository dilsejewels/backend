...
class PaymentController extends Controller
{
    ...
    //         // 3. SAVE TO DATABASE
    //         // The frontend sends 'items' and 'address' from LocalStorage in this request
    //         $order = Order::create([
    ...
    //             // 2. PREPARE DATA (Logic copied from your PayPal Function)
    //             $payload = $request->items ?? [];
    ...
    //             foreach ($payload as $item) {
    //                 $type = $item['productType'] ?? null;
    //                 // (Keep your existing switch case logic here to fill $itemsId)
    //                 // ... [Same logic as PayPal] ...
    //                 if($type === 'diamond' && !empty($item['diamondid'])) $itemsId['diamond'][] = $item['diamondid'];
    //                 // ... etc ...
    //             }

    //             // ... Determine Product Type ...
    //             $productType = 'multiple'; // (Simplified for brevity, use your PayPal logic)
    ...
    //             // 3. SAVE TO DB (Matching your PayPal Schema)
    //             $order = Order::create([
    //                 'order_id'         => 'ORD-' . Str::uuid(),
    //                 'user_id'          => $request->user()->id ?? null,
    //                 'user_name'        => $userName,
    ...
    //                 'billing_address'  => json_encode($billing, JSON_UNESCAPED_SLASHES),
    //                 'total_price'      => $request->total_amount,
    //                 'payment_mode'     => 'cashfree',     // <--- Changed
    //                 'payment_method'   => 'cashfree',     // <--- Changed
    //                 'payment_status'   => 'paid',
                    
    //                 // Frontend sent "discount_amount", but your DB uses "coupon_discount"
    //                 'coupon_code'      => $request->coupon_code,
    //                 'coupon_discount'  => $request->discount, 

    //                 'transaction_id'   => $orderId,       // <--- Cashfree Order ID
